...
from difang.majiang2.ting_rule.ting_rule_factory import MTingRuleFactory
...
class MajiangTableLogic(AbstractMajiangTableLogic):
    def __init__(self, playerCount, playMode, runMode):
        ...
        if self.checkTableState(MTableState.TABLE_STATE_TING):
            ...
            self.__ting_rule_mgr.setTableTileMgr(self.__table_tile_mgr)
        else:
        ...
        # 用于存储每人网络状况的字典
        self.__player_ping = {}
        # 昭通胡牌但牌局未结束时可以补发杠,这里用来保存杠牌信息
        self.__gang_after_hu = {}
        ftlog.debug('test')
    ...
    def checkBeginHu(self, tile):
        # 检查第一次发牌之后就和牌的情况
        if self.__action_id > 1:
            return False

        ftlog.debug('MTableLogic.checkBeginHu tile:', tile)
        ...
        if self.playMode == MPlayMode.JIXI:
            ...
            # 用听牌规则判断 庄家可以听就直接胡牌
            isTing, tingArr = self.tingRule.canTing(playerAllTiles[self.__cur_seat], self.tableTileMgr.tiles, tile)
            if isTing:
                isHu = False
                ...
                if isHu:
                    ...
                    self.__add_card_processor.initProcessor(self.actionID, state, self.__cur_seat, tile, exInfo,
                                                            timeOut)
                    # 直接胡牌
                    self.gameWin(self.__cur_seat, tile)
                    return True

            if self.playerCount == 3:
                isHu, huSeatId = self.__win_rule_mgr.isWuDuiHu(playerAllTiles, tile)
                if isHu:
                    self.tableTileMgr.updateMagicTile();
                    self.__cur_seat = huSeatId
                    state = MTableState.TABLE_STATE_HU
                    exInfo = MTableStateExtendInfo()
                    winInfo = {}
                    winInfo['tile'] = tile
    ...
    def processAddTile(self, cp, state, special_tile=None):
        ...
        if state == MTableState.TABLE_STATE_NEXT and cp.isTing():
            # 哈尔滨宝中宝
            magicTiles = self.tableTileMgr.getMagicTiles(cp.isTing())
            if self.__win_rule_mgr.isBaoZhongBaoHu(cp.isTing(), cp.handTiles, magicTiles):
                # 扩展数据
                state = MTableState.TABLE_STATE_HU
                exInfo = MTableStateExtendInfo()
                winInfo = {}
                if self.haveRestTile():
                    ...
                    return
                winInfo['tile'] = tile
                winInfo['baoZhongBao'] = 1
                exInfo.appendInfo(state, winInfo)
                timeOut = self.__table_stater.getTimeOutByState(state)
                self.__add_card_processor.initProcessor(self.actionID, state, cp.curSeatId, tile, exInfo, timeOut)
                # 直接胡牌
                self.gameWin(cp.curSeatId, tile)
                return

            # 鸡西听后如果胡的是宝牌则直接胡(通宝／宝边 宝中宝／宝夹）
            if self.__win_rule_mgr.isMagicAfertTingHu(cp.isTing(), cp.winNodes, magicTiles):
                # 扩展数据
                state = MTableState.TABLE_STATE_HU
                exInfo = MTableStateExtendInfo()
                winInfo = {}
        ...
        if self.tableConfig.get(MTDefine.GUA_DA_FENG, 0) and cp.isTing():
            ...
            ftlog.debug('MajiangTableLogic.processAddTile check GUA_DA_FENG tiles:', tiles, 'gangs:', gangs)
            # 摸到自己的碰牌和手里的刻牌(可杠)
            if len(gangs) > 0:
                state = MTableState.TABLE_STATE_HU
                exInfo = MTableStateExtendInfo()
        ...
        if self.checkTableState(MTableState.TABLE_STATE_TING):
            if not cp.isTing():
                ...
                ftlog.debug('MajiangTableLogic.processAddTile canTing result: ', tingResult, ' solution:', tingReArr,
                            ' length: ', len(tingReArr))
    ...
    def getCreateExtendBudgets(self, score):
        ...
        for seatId in range(self.playerCount):
            ...
            if self.playMode == MPlayMode.HAERBIN:
                statisticInfo.append(
                    {"desc": oneResultForName.statType[MOneResult.STAT_MINGGANG]["name"], "value": mingGangValue})
                ftlog.debug('MTableLogic.createExtendBudgets seatId', seatId, ' mingGangValue:', mingGangValue)
                statisticInfo.append(
                    {"desc": oneResultForName.statType[MOneResult.STAT_ANGANG]["name"], "value": anGangValue})
                ftlog.debug('MTableLogic.createExtendBudgets seatId', seatId, ' anGangValue:', anGangValue)
            if self.playMode != MPlayMode.YUNNAN and self.playMode != MPlayMode.ZHAOTONG:
                statisticInfo.append(
                    {"desc": oneResultForName.statType[MOneResult.STAT_ZUIDAFAN]["name"], "value": zuidaFanValue})
    ...
    def gameNext(self):
        ...
        if self.playMode == MPlayMode.JIXI:
            ...
            # 有人听牌 初始化第一张宝牌
            if len(magics) == 0 and playerTing:
                self.tableTileMgr.updateMagicTile()
        ...
        if self.__drop_card_processor.updateProcessor(self.actionID, seatId, state, chiTile, chiPattern):
            if self.__drop_card_processor.allResponsed():
                ...
                if state & MTableState.TABLE_STATE_GRABTING:
                    self.__add_card_processor.initProcessor(self.actionID, MTableState.TABLE_STATE_TING, cp.curSeatId,
                                                            chiTile, exInfo, timeOut)
                else:
                    self.addCardProcessor.initProcessor(self.actionID, MTableState.TABLE_STATE_DROP, cp.curSeatId,
                                                        chiTile, exInfo, timeOut)
        ...
        if self.__drop_card_processor.updateProcessor(self.actionID, seatId, state, tile, pengPattern):
            if self.__drop_card_processor.allResponsed():
                ...
                if state & MTableState.TABLE_STATE_GRABTING:
                    ...
                    self.__add_card_processor.initProcessor(self.actionID, MTableState.TABLE_STATE_DROP, cp.curSeatId,
                                                            tile, exInfo, timeOut)
    ...
    def zhanTile(self, seatId, tile, zhanPattern, state, special_tile):
        ...
        if self.__drop_card_processor.updateProcessor(self.actionID, seatId, state, tile, zhanPattern):
            ...
            # 初始化处理器，抢听完了转听
            if state & MTableState.TABLE_STATE_GRABTING:
                self.__add_card_processor.initProcessor(self.actionID, MTableState.TABLE_STATE_TING, cp.curSeatId, tile,
                                                        exInfo, timeOut)
            else:
                    ...
                    if not self.__drop_card_processor.allResponsed():
                        ...
                        if self.playMode == MPlayMode.ZHAOTONG:
                            ...
                            for index in turns:
                                ...
                                ftlog.debug('MajiangTableLogic.gameWin__gang_after_hu:'
                                            , self.__gang_after_hu
                                            , 'turns:', turns
                                            , 'state:', state
                                            , 'exInfo:', exInfo
                            ...
                            #                                     gang, newState = exInfo.getFirstPattern(MTableState.TABLE_STATE_GANG, self.tableTileMgr.getMagicTiles())
                            #                                     gangSeatId = index
                            #                                     gangState = newState
                            #                                     gangTile = self.dropCardProcessor.tile
                            #                                     gangStyle = gang['style']
                            #                                     gangPattern = gang['pattern']
                            #                                     self.__gang_after_hu = {
                            #                                         'lastSeatId':self.__drop_card_processor.curSeatId,
                            #                                         'seatId':gangSeatId,
                            #                                         'state':gangState,
                            #                                         'tile':gangTile,
                            #                                         'style':1,
                            #                                         'pattern':gangPattern
                            #                                         }
                            #                                 ftlog.debug('MajiangTableLogic.gameWin__gang_after_hu:', self.__gang_after_hu)
                            #                         self.__drop_card_processor.reset()
                        if self.playMode == MPlayMode.ZHAOTONG:
    ...
    def processHu(self, player):
        ...
        if self.checkTableState(MTableState.TABLE_STATE_XUEZHAN):
            ...
            if self.getWinPlayerCount() >= self.playerCount - 1:
                ...
                #                                         }
                self.__win_seats = []
                # 如果是昭通,还没完局,有人能杠,就把这个杠补发下去
                if self.playMode == MPlayMode.ZHAOTONG:
                    if self.__gang_after_hu.has_key('seatId'):
                        self.__gangTile(self.__gang_after_hu.get('lastSeatId', -1)
                                        , self.__gang_after_hu.get('seatId', -1)
                                        , self.__gang_after_hu.get('tile', 0)
                                        , self.__gang_after_hu.get('pattern', [])
                                        , self.__gang_after_hu.get('gangStyle', 1)
                                        , self.__gang_after_hu.get('state', 0)
                                        , False)
                        self.__gang_after_hu = {}
                    else:
                        if self.__drop_card_processor.getState() == 0:
                            ...
                            for i in range(self.playerCount):
                                if self.player[i].hasHu:
                                    continue
                                state = self.__drop_card_processor.getStateBySeatId(i)
                                ftlog.debug('MajiangTableLogic.processHu noHu state', state)
    ...
    def refixTableStateByConfig(self):
        """根据自建房配置调整牌桌状态"""
        itemParams = self.getTableConfig(MFTDefine.ITEMPARAMS, {})
        ftlog.info("refixTableStateByConfig itemParams=", itemParams)
        chipengSetting = itemParams.get('chipengsetting', 0)
        ftlog.info("refixTableStateByConfig ", chipengSetting)
        if chipengSetting == 2:
    ...
    def refixTableMultipleByConfig(self):
        """根据传入配置调整输赢倍数"""
        itemParams = self.getTableConfig(MFTDefine.ITEMPARAMS, {})
        ftlog.info("refixTableMutipleByConfig itemParams=", itemParams)
        multiple = itemParams.get('multiple', 1)
        ftlog.info("refixTableMutipleByConfig ", multiple)
    ...
    def refixTableZimoBonusByConfig(self):
        """根据传入配置调整自摸加番"""
        itemParams = self.getTableConfig(MFTDefine.ITEMPARAMS, {})
        ftlog.info("refixTableZimoBonusByConfig itemParams=", itemParams)
        zimoBonus = itemParams.get('zimoBonus', 1)
        ftlog.info("refixTableZimoBonusByConfig ", zimoBonus)
...
if __name__ == "__main__":
    ...
    table = MajiangTableLogic(2, MPlayMode.ZHAOTONG, MRunMode.CONSOLE)
    ...
    table.addPlayer(player0, 0, True, True)

    player1 = MPlayer('1', 1, 10001, 0)
    table.addPlayer(player1, 1, True, True)
