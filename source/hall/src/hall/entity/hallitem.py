...
class TYItemBindingsException(TYItemException):
    ...
    def __str__(self):
        return '%s:%s %s:%s %s' % (self.errorCode, self.message, self.item.itemId, self.item.kindId,
                                   [(ci.assetKindId, ci.count) for ci in self.itemBindings.items])

    def __unicode__(self):
        return u'%s:%s %s:%s %s' % (self.errorCode, self.message, self.item.itemId, self.item.kindId,
                                    [(ci.assetKindId, ci.count) for ci in self.itemBindings.items])
...
def buildMailAndMessageAndChanged(gameId, userAssets, action, assetList, replaceParams):
    ...
    if mail:
        pkmessage.send(gameId, pkmessage.MESSAGE_TYPE_SYSTEM, userAssets.userId, mail)
        changed.add('message')
...
class TYItemActionAssemble(TYItemAction):
    ...
    def _doActionImpl(self, gameId, userAssets, item, timestamp, params):
        ...
        for component in item.itemKind.componentOf.componentList:
            actually = userBag.calcTotalUnitsCount(component.itemKind, timestamp)
            if actually < component.count:
                raise TYItemKindAssembleNotEnoughException(item.itemKind.componentOf, component.itemKind,
                                                           component.count, actually)
...
class TYItemActionPresent(TYItemAction):
    ...
    def _doActionImpl(self, gameId, userAssets, item, timestamp, params):
        ...
        if item.itemKind.singleMode:
            ...
            user_remote.presentItemByUnitsCount(gameId, targetUserId, userAssets.userId, item.kindId, count,
                                                receiveMail)
...
class TYItemActionBoxOpen(TYItemAction):
    ...
    def _doActionImpl(self, gameId, userAssets, item, timestamp, params):
        ...
        _mail, message, _changed = _handleMailAndMessageAndChanged(gameId, userAssets, self, assetItemList,
                                                                   replaceParams)
...
class TYItemActionExchange(TYItemAction):
    ...
    def _doActionImpl(self, gameId, userAssets, item, timestamp, params):
        ...
        record = hallexchange.requestExchange(userAssets.userId, item, exchangeParams, timestamp)
        ...
        return TYItemExchangeResult(self, item, message, record.exchangeId)
...
class TYItemActionWeixinRedEnvelopGetPassword(TYItemAction):
    ...
    def _doActionImpl(self, gameId, userAssets, item, timestamp, params):
        ...
        if ec == 0:
            ...
            userBag = userAssets.getUserBag()
            userBag.removeItem(gameId, item, timestamp, 'ITEM_USE', 0)
            _mail, message, _changed = _handleMailAndMessageAndChanged(gameId, userAssets, self, None,
                                                                       {'password': result})
...
def _registerClasses():
    ...
    TYItemActionConditionRegister.registerClass(TYItemActionConditionLeastOncePay.TYPE_ID,
                                                TYItemActionConditionLeastOncePay)
    TYItemActionConditionRegister.registerClass(TYItemActionConditionLeastOnceAlipay.TYPE_ID,
                                                TYItemActionConditionLeastOnceAlipay)
    TYItemActionConditionRegister.registerClass(TYItemActionConditionGotSecondDaysLater.TYPE_ID,
                                                TYItemActionConditionGotSecondDaysLater)
    TYItemActionConditionRegister.registerClass(TYItemActionConditionBindPhoneCheck.TYPE_ID,
                                                TYItemActionConditionBindPhoneCheck)
    TYItemActionConditionRegister.registerClass(TYItemActionConditionBindWeixin.TYPE_ID,
                                                TYItemActionConditionBindWeixin)
    TYItemActionConditionRegister.registerClass(TYItemActionConditionCanOpenFlag.TYPE_ID,
                                                TYItemActionConditionCanOpenFlag)
    TYItemActionConditionRegister.registerClass(ItemActionConditionGameDashifenLevel.TYPE_ID,
                                                ItemActionConditionGameDashifenLevel)
    ...
    TYItemActionRegister.registerClass(TYItemActionWeixinRedEnvelopGetPassword.TYPE_ID,
                                       TYItemActionWeixinRedEnvelopGetPassword)
    ...
    if isCreate:
        ...
        if pkgamedata.setnxGameAttr(userId, HALL_GAMEID, 'flag.item.trans', 1) == 1:
            if not _tranformItems(gameId, userId, clientId, userBag):
                _initUserBag(gameId, userId, clientId, userBag)
        hallsubmember.checkSubMember(gameId, userId, timestamp, '', 0, userAssets)
