...
from hall.entity.todotask import TodoTaskRegister, TodoTaskFactory, \
    ...
    TodoTaskOrderShow, TodoTaskDiZhuEvent
...
class TodoTaskTemplate(TodoTaskFactory):
    ...
    def newTodoTask(self, gameId, userId, clientId, **kwargs):
        ...
        if self.conditionList:
            ok, cond = self._checkConditions(gameId, userId, clientId, timestamp, self.conditionList)
            if ftlog.is_debug():
    ...
    def _newTodoTask(self, gameId, userId, clientId, timestamp, **kwargs):
        if not hallstore.isFirstRecharged(userId):
            ...
            if product:
                ...
                return TodoTaskFirstRecharge(self.title, self.imgUrl, payOrder, self.confirm, self.confirmDesc,
                                             self.subText)
...
class TodoTaskMemberBuyTemplate(TodoTaskTemplate):
    ...
    def _newTodoTask(self, gameId, userId, clientId, timestamp, **kwargs):
        return TodoTaskMemberBuy(self.desc, self.pic)
...
class TodoTaskMemberTryTemplate(TodoTaskTemplate):
    ...
    def _newTodoTask(self, gameId, userId, clientId, timestamp, **kwargs):
        return TodoTaskMemberTry(self.desc, self.pic)
...
class TodoTaskLessbuyChipTemplate(TodoTaskTemplate):
    ...
    def _newTodoTask(self, gameId, userId, clientId, timestamp, **kwargs):
        ...
        roomMinChip = gdata.getRoomMinCoin(roomId)
        ...
        if not product:
            ftlog.warn('TodoTaskLessbuyChipTemplate._newTodoTask gameId=', gameId,
                       ...
                       'err=', 'NoProduct')
            return None
        desc = strutil.replaceParams(self.desc,
                                     {'room.minCoin': str(roomMinChip), 'product.displayName': product.displayName})
        note = strutil.replaceParams(self.note, {'product.price': str(product.price)})
        ret = TodoTaskLessBuyOrder(desc, self.iconPic, note, None)
        if product:
            payOrder = TodoTaskHelper.makeTodoTaskPayOrder(userId, clientId, product)
    ...
    def _newTodoTask(self, gameId, userId, clientId, timestamp, **kwargs):
        ...
        if ftlog.is_debug():
            ftlog.debug('TodoTaskRoomBuyTemplate._newTodoTask gameId=', gameId,
                        ...
                        'kwargs=', kwargs)
        if roomId:
            product, _ = hallproductselector.selectProcutByRoomId(gameId, userId, clientId, roomId,
                                                                  self.selectProductAction)
            if product:
...
def makeTodoTaskLessbuyChip(gameId, userId, clientId, roomId):
    from hall.entity import hallproductselector
    if ftlog.is_debug():
    ...
    if clientVer >= 3.7:
        ...
        if ftlog.is_debug():
            ftlog.debug('hallpopwnd.makeTodoTaskLessbuyChip gameId=', gameId,
                        ...
                        'template=', template)
        if template:
            return template.newTodoTask(gameId, userId, clientId, roomId=roomId)
    else:
        product, _ = hallproductselector.selectLessbuyProduct(gameId, userId, clientId, roomId)
        if product:
            if clientOs == 'winpc':
...
class TodoTasksGeneratorRegister(TYConfableRegister):
    _typeid_clz_map = {
        TodoTasksGeneratorList.TYPE_ID: TodoTasksGeneratorList,
        TodoTasksGeneratorSwitch.TYPE_ID: TodoTasksGeneratorSwitch,
        TodoTasksGeneratorSingle.TYPE_ID: TodoTasksGeneratorSingle
    }
