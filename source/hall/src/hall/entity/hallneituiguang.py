...
from hall.entity import hallconf, hallaccount
...
class NeiTuiGuangProtocolBuilder(object):
    ...
    @classmethod
    def buildOldUserState(cls, gameId, userId, action, state, bindok, shareId, prizeinfo, weixintip, url, smstip, rules,
                          mo):
        mo.setCmd('promote_info')
        mo.setResult('gameId', gameId)
        mo.setResult('state', state)
...
class NeiTuiGuang(object):
    ...
    def getSMSTip(cls, gameId, userId, conf):
        conf = conf.get('gameDoc', {})
        return strutil.cloneData(conf.get('smsInviteDoc')).format(PROMOTE_CODE=userId,
                                                                  DOWNLOAD_URL=cls.getDownUrl(userId, conf))
