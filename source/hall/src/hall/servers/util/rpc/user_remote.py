...
def consumeAsset(gameId, userId, assetKindId, count, eventId, intEventParam):
    try:
        ...
        _, consumeCount, final = userAssets.consumeAsset(gameId, assetKindId, count, pktimestamp.getCurrentTimestamp(),
...
def addAsset(gameId, userId, assetKindId, count, eventId, intEventParam):
    try:
        userAssets = hallitem.itemSystem.loadUserAssets(userId)
        _, addCount, final = userAssets.addAsset(gameId, assetKindId, count, pktimestamp.getCurrentTimestamp(), eventId,
                                                 intEventParam)
...
def sendHallLed(gameId, msgstr, ismgr=0, scope='hall', clientIds=[]):
    count = 0
    colen = router._utilServer.sidlen
    for x in xrange(colen):
        #         count += _sendHallLed(x + 1, gameId, msgstr, ismgr, scope, clientIds)
        _sendHallLed(x + 1, gameId, msgstr, ismgr, scope, clientIds)
    return count
