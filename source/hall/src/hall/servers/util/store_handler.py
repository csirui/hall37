...
class StoreTcpHandler(BaseMsgPackChecker):
    ...
    @classmethod
    def deliveryProduct(cls, gameId, userId, orderId, prodId,
                        chargeType, chargeMap, consumeMap, isSub=False):
        orderId = cls.checkCompensateOrderId(gameId, userId, orderId, prodId)
        try:
    ...
    def doDeliveryProduct(self, gameId, userId, orderId, prodId,
                          chargeType, chargeMap, consumeMap):
        msg = runcmd.getMsgPack()
        isSub = msg.getParam('isSub', 0)
        return self.deliveryProduct(gameId, userId, orderId, prodId, chargeType, chargeMap, consumeMap, isSub)
