...
from webmgr.handler.game.t3flush import room_conf_generator
from tyserver.tyutils.cron import FTCron
import tyserver.tyutils.tylog as ftlog


svnroot = 'http://202.106.9.134:9055/newsvn'
roomConfTestingUrl = svnroot + '/config/trunk/test/game/39/room'
roomConfReleaseUrl = svnroot + '/config/branches/online-release/game/39/room'
teamConfUrl = svnroot + '/team-t3flush/config'
...
class T3flushActionHandler(object):
    ...
    def do_http_t3flush_commit(self, branch, commitlog):
        self._initBase()
        ftlog.debug("do_http_t3flush_commit branch, commitlog:", branch, commitlog)
        if branch == 'testing':
            src = teamCfgDir
            tgt = testingDir
        elif branch == 'release':
        ...
        ftlog.debug("do_http_t3flush_commit branch, tgt, commitlog, :", branch, tgt, commitlog)
    ...
    def _initBase(self):
        ...
        ftlog.debug('T3flushActionHandler._initBase >>|', 'projects_path, teamRoot, runningDir:',
    ...
    def _svnCmd(self, workingpath, svnCmd, *svnArgs):
        ...
        ftlog.debug('T3flushActionHandler._svnCmd >>| cmd, status, output:', cmd, status, output)
    ...
    def do_http_t3flush_get_match_time(self):
        ...
        ftlog.debug("T3flushActionHandler.do_http_t3flush_get_match_time", "days:", days)
