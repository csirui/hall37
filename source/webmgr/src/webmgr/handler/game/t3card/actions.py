...
from webmgr.handler.game.t3card import room_conf_generator
from tyserver.tyutils.cron import FTCron
import tyserver.tyutils.tylog as ftlog


svnroot = 'http://202.106.9.134:9055/newsvn'
roomConfTestingUrl = svnroot + '/config/trunk/test/game/30/room'
roomConfReleaseUrl = svnroot + '/config/branches/online-release/game/30/room'
teamConfUrl = svnroot + '/team-t3card/config'
...
class T3cardActionHandler(object):
    
    def __init__(self, options):
        self.options = options

    @markHttpRequestEntry(jsonp=1)
    def do_http_t3card_gen_json_file(self, jsonfile):
        self._initBase()
        ftlog.debug('do_http_t3card_gen_json_file', jsonfile)
        if jsonfile == 'game/30/room/0.json':
            return self.gen_t3card_room_json()
        
    @markHttpRequestEntry(jsonp=1)
    def do_http_t3card_get_json_file(self, jsonfile):
        self._initBase()
        ftlog.debug("do_http_t3card_gen_json_file jsonfile:", jsonfile)
    ...
    def do_http_t3card_precommit(self, branch):
        self._initBase()
        ftlog.debug("do_http_t3card_precommit branch:", branch)
        if branch == 'testing':
    ...
    @markHttpRequestEntry()
    def do_http_t3card_commit(self, branch, commitlog):
        self._initBase()
        ftlog.debug("do_http_t3card_commit branch, commitlog:", branch, commitlog)
        if branch == 'testing':
            src = teamCfgDir
            tgt = testingDir
        elif branch == 'release':
        ...
        ftlog.debug("do_http_t3card_commit branch, tgt, commitlog, :", branch, tgt, commitlog)
        cmd, status, output = self._svnCmd(teamRoot, 'commit', tgt, '-m', '"%s"' % commitlog)
        ftlog.debug("do_http_t3card_commit| branch:", branch,
    ...
    def _initBase(self):
        ...
        if getattr(self.options, 't3cardTeamConfInited', False):
            return
        ...
        teamRoot = os.path.join(projects_path, 't3card-team')
        runningDir = os.path.join(pokerpath, 'game/30/room/')

        ftlog.debug('T3cardActionHandler._initBase >>|', 'projects_path, teamRoot, runningDir:',
        ...
        self.options.t3cardTeamConfInited = True
    
    def _svnCmd(self, workingpath, svnCmd, *svnArgs):
        print 'DEBUG t3cardActionHandler._svnCmd <<| workingpath, svnCmd, svnArgs', workingpath, svnCmd, svnArgs
        ...
        ftlog.debug('T3cardActionHandler._svnCmd >>| cmd, status, output:', cmd, status, output)
        return cmd, status, output
    
    def gen_t3card_room_json(self):
        self._initBase()
    ...
    @markHttpRequestEntry(jsonp=1)
    def do_http_t3card_get_match_time(self):
        '''获取所有比赛的所有场次的所有开赛时间'''
        self._initBase()
        days = int(runhttp.getParamStr('days', 5))
        ftlog.debug("T3cardActionHandler.do_http_t3card_get_match_time", "days:", days)
