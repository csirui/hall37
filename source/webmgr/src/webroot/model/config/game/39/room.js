...
// 加载菜单请求
var jsonfile = 'game/39/room/0.json';
var jsonvue = null;
var mindex = 1;
var metaData = {};
var selected_rows = [];
...
function generate_json() {
    ...
    SS.getData(SS.url.model.t3flush_gen_json_file, params,
    function(da) {
        var htmls = [];
        if (da.result.isOk) {
            ...
            htmls.push('<iframe name="diff_room_iframe" frameborder="0"  style="width:100%;height:100%;" src='
              ...
              + '&left=' + SS.host + '/t3flush/get/json/file?jsonfile=' + encodeURIComponent(jsonfileprev)
              + '&right='  + SS.host + '/t3flush/get/json/file?jsonfile=' + encodeURIComponent(jsonfile)
              + '"'
              + '>'
              + '</iframe>'
              );
...
function real_commit () {
    ...
    SS.setData(SS.url.model.t3flush_commit, params, function(da) {
        var htmls = [];
...
function commit(branch) {
    ...
    SS.getData(SS.url.model.t3flush_precommit, params,
    function(da) {
        var htmls = [];
        var jsonfile = da.result.jsonfile;
        var jsonfileprev = da.result.jsonfileprev;
