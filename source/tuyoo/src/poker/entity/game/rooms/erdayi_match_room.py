...
class TYErdayiMatchRoom(TYRoom):
    ...
    def doLeave(self, userId, msg):
        if self._logger.isDebug():
            self._logger.debug('TYErdayiMatchRoom.doLeave',
                               'userId=', userId)
        mo = MsgPack()
        mo.setCmd('m_leave')
        mo.setResult('gameId', self.gameId)
    ...
    def doGetDescription(self, userId):
        if self._logger.isDebug():
            self._logger.debug('TYErdayiMatchRoom.doGetDescription',
                               'userId=', userId)
        mo = MsgPack()
        mo.setCmd('m_des')
        mo.setResult('gameId', self.gameId)
    ...
    def doUpdateInfo(self, userId):
        if self._logger.isDebug():
            self._logger.debug('TYErdayiMatchRoom.doUpdateInfo',
                               'userId=', userId)
    ...
    def doSignout(self, userId):
        if self._logger.isDebug():
            self._logger.debug('TYErdayiMatchRoom.doSignout',
                               'userId=', userId)
    ...
    def doWinlose(self, msg):
        ...
        if self._logger.isDebug():
            self._logger.debug('TYErdayiMatchRoom.doWinlose',
                               'mid=', mid,
                               'tableId=', tableId,
                               'ccrc=', ccrc)
    ...
    def doQuickStart(self, msg):
        ...
        self._logger.info('TYErdayiMatchRoom.doQuickStart',
                          'userId=', userId,
                          'tableId=', tableId,
        ...
        if tableId == self.roomId * 10000:
            ...
            player = self.match.findPlayer(userId)
            if player is None:
                self._logger.warn('TYErdayiMatchRoom.doQuickStart',
                                  'userId=', userId,
                                  'tableId=', tableId,
                                  'shadowRoomId=', shadowRoomId,
                                  'clientId=', clientId,
