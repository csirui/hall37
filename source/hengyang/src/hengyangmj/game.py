...
from hengyangmj.entity.configure.conf import GAMEID
from hengyangmj.table.majiang_friend_table import HYMajiangFriendTable
from hengyangmj.table.majiang_quick_table import HYMajiangQuickTable
...
class TGhengyangmj(TYGame):
    def __init__(self, *args):
        super(TGhengyangmj, self).__init__()
        self._lastSendIntervalLedTime = int(time.time())  # 上一次下发'每隔多久LED'LED的时间

    def initGameBefore(self):
        from hengyangmj.entity import task_inspectors
    ...
    def newTable(self, room, tableId):
        ...
        if ftlog.is_debug():
            ftlog.debug('TGhengyangmj.newTable playMode:', playMode, ' isCreateTable:', isCreateTable)

        # 根据playMode和isCreateTable决定实例化那张桌子
        if isCreateTable:
            table = HYMajiangFriendTable(tableId, room)
        else:
            table = HYMajiangQuickTable(tableId, room)
        return table
