...
class hengyangmjGameTcpHandler(GameTcpHandler):
    def __init__(self):
        super(hengyangmjGameTcpHandler, self).__init__()

    @markCmdActionMethod(cmd='game', action="room_list", clientIdVer=0, scope='game', lockParamName="")
    def doRoomList(self, userId, gameId):
        super(hengyangmjGameTcpHandler, self).doRoomList(userId, gameId)

    @markCmdActionMethod(cmd='user', action='mj_timestamp', scope='game')
    def curTimestemp(self, gameId, userId):
        super(hengyangmjGameTcpHandler, self).curTimestemp(gameId, userId)
    ...
    def getUserInfoSimple(self, userId, gameId, roomId0, tableId0, clientId):
        super(hengyangmjGameTcpHandler, self).getUserInfoSimple(userId, gameId, roomId0, tableId0, clientId)
    ...
    def doGetMajiangItem(self, gameId, userId, clientId):
        super(hengyangmjGameTcpHandler, self).doGetMajiangItem(gameId, userId, clientId)
